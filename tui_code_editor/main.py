"""Main entrypoint for the TUI code editor using prompt_toolkit."""
...
class Tab:
    def __init__(self, path: Optional[Path], text: str = "", search_toolbar: Optional[SearchToolbar] = None, on_change=None):
        ...
        try:
            # Buffer exposes an event we can attach to
            self.buffer.on_text_changed += _on_change
            if on_change is not None:
                # also call external on_change callback
                def _external(_):
                    try:
                        on_change(self)
                    except Exception:
            ...
            # older/newer prompt_toolkit may differ; ignore if unavailable
            pass

        # Create the TextArea; enable line numbers and connect search toolbar if provided
        ...
        # add simple completer (scaffold)
        try:
            from prompt_toolkit.completion import Completer

            class BufferCompleter(Completer):
                def get_completions(self, document, complete_event):
                    # simple completions: words from the buffer
...
class EditorApp:
    def __init__(self, root: Path):
        ...
        # single-line input for terminal; when Enter is pressed, it will send to PTY
        ...
        @kb.add("c-o")
        def _(event):
            """Open file under cursor in explorer"""
        ...
        @kb.add("c-f")
        def _(event):
            # focus the search toolbar
            event.app.layout.focus(self.search_toolbar.control)

        @kb.add("c-n")
        def _(event):
            """Create new file under cwd (prompt for path)"""
            try:
        ...
        @kb.add("c-r")
        def _(event):
            """Rename selected file in explorer"""
            try:
        ...
        def _(event):
            ...
            try:
                ...
                # message_dialog.run() returns None; use simple confirmation via input_dialog
        ...
        @kb.add("c-`")
        def _(event):
            # focus terminal input and start session if needed
        ...
        @kb.add("c-space")
        def _(event):
            # request completions from LSP and show choices
        ...
        def _(event):
            ...
            self._save_config()
            # a simple effect: swap status style
            if self.theme == "dark":
                self.style = Style.from_dict({
                    "frame.border": "ansiblue",
                    "frame.label": "bold",
                    "status": "reverse",
                    "editor.line-number": "ansidarkgray",
                    "editor.current-line": "bg:#1e1e1e",
                    "terminal": "bg:#0b0b0b #ffffff",
                })
            else:
                self.style = Style.from_dict({
                    "frame.border": "ansiblue",
                    "frame.label": "bold",
                    "status": "reverse",
                    "editor.line-number": "ansiblack",
                    "editor.current-line": "bg:#ffffff",
                    "terminal": "bg:#f8f8f8 #000000",
                })
            self.application.style = self.style

        self.kb = kb

        # Top: explorer + editor. Bottom: status + terminal
        root_container = HSplit([
            VSplit([
                Frame(self.file_explorer, title="Explorer"),
                HSplit([
                    Frame(self.editor_frame, title="Editor", width=100),
                    self.search_toolbar,
                ]),
            ]),
            VSplit([self.status]),
            Frame(self.terminal_output, title="Terminal"),
        ])
        ...
        # wire terminal input accept handler
        def _accept_terminal(buff):
            ...
            if text:
                try:
                    ...
                    # append error to terminal output so user sees failure
                    try:
    ...
    def _render_current(self):
        ...
        # Build a tabs bar showing filenames and dirty state
        tabs_labels = []
    ...
    async def _lsp_completion(self):
        """Request LSP completions and show a selection dialog to pick one."""
        ...
        if not res:
            return
        # completion result can be list or dict with 'items'
        ...
        for it in raw_items:
            ...
            if isinstance(it, dict):
                ...
                # if textEdit provided prefer that
                te = it.get("textEdit")
                if te and isinstance(te, dict):
                    insert = te.get("newText", insert)
            choices.append((label, insert))
        if not choices:
            self.status.text = "No completions"
            return
        # use radiolist_dialog for selection
        try:
    ...
    def save_current(self):
        ...
        if not tab.path:
            return
        try:
            # If python file, try to format with black (stdin -> stdout)
            ...
            # notify LSP (didSave) if available
            if self.lsp:
    ...
    def search_in_current(self):
        if not self.open_tabs:
            return
        # We now use the SearchToolbar provided by prompt_toolkit. This method remains for backwards compatibility.
        pass
    ...
    async def _start_lsp_for_current(self):
        # Try to start pylsp if file is python and pylsp exists
        if not self.open_tabs:
...
def main():
    # Ensure we are running in a proper terminal. prompt_toolkit requires a real TTY.
